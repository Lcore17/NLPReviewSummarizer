...
import pickle
from ml_models import load_data, preprocess_text as ml_preprocess, generate_features, train_evaluate_model
...
def run_ml_pipeline(data_path):
    """Run the complete ML pipeline from ml_models.py"""
    df, dataset_type, target_col, text_col, class_names = load_data(data_path)
    
    # Handle column name mismatch
    if 'review' in df.columns and text_col != 'review':
        text_col = 'review'
        st.write(f"Updated text column to 'review' for ML pipeline compatibility")

    # Perform preprocessing
    df['cleaned_text'] = df[text_col].apply(
        lambda x: ml_preprocess(x, dataset_type))
    
    # Encode target labels for Amazon dataset
    if dataset_type == 'amazon':
    ...
    results = {}
    text_reports = defaultdict(dict)  # Initialize text_reports collection
    ...
    for feature_name, X in feature_sets.items():
        ...
        for model in models:
            ...
            # Store text reports
            text_reports[feature_name][model.__class__.__name__] = metrics['text_report']
...
def main():
    ...
    with st.sidebar:
        ...
        enable_ml = st.checkbox("Enable ML Model Analysis")
    ...
    if uploaded_file:
        ...
        if enable_ml:
            ...
            with st.spinner("Running machine learning pipeline..."):
                ...
                pipeline_results = run_ml_pipeline("temp_dataset.csv")
                ml_results = pipeline_results['results']
                class_names = pipeline_results['class_names']
                text_reports = pipeline_results['text_reports']
                ...
                comparison = []
                for feature_name, models in ml_results.items():
                ...
                best_feature = best_model_info['Feature Set']
                best_model = best_model_info['Model']
                
                conf_matrix = ml_results[best_feature][best_model]['confusion_matrix']
                plt.figure(figsize=(8, 6))
                sns.heatmap(conf_matrix, annot=True, fmt='d',
                            cmap='Blues', xticklabels=class_names, yticklabels=class_names)
                plt.title(f"{best_model} Confusion Matrix ({best_feature})")
                st.pyplot(plt)
                ...
                # Create tabs for each feature set
                feature_tabs = st.tabs(list(ml_results.keys()))
                
                for tab_idx, feature_name in enumerate(ml_results.keys()):
                    with feature_tabs[tab_idx]:
                        st.markdown(f"### {feature_name} Features")
                        
                        # Create columns for model selection and reports
                        col1, col2 = st.columns([1, 3])
                        
                        with col1:
                            selected_model = st.selectbox(
                                "Choose Model",
                                options=list(ml_results[feature_name].keys()),
                                key=f"model_sel_{feature_name}"
                            )
                        
                        with col2:
                            metrics = ml_results[feature_name][selected_model]
                            ...
                            with st.expander("Detailed Metrics", expanded=False):
                                report_df = pd.DataFrame(
                                    metrics['classification_report']).transpose()
                                st.dataframe(
                                    report_df.style.format("{:.2%}", subset=['precision', 'recall', 'f1-score'])
